$(function(){
    var slider = $( '.ares-slider1' );

    new AresSlider3( {
        obj: slider,
        items: slider.find( '>.ares-slider1__item' ),
        btnNext: slider.find( '.ares-slider1__next' ),
        btnPrev: slider.find('.ares-slider1__prev')
    } );
});

var AresSlider3 = function( params ){
    this.obj = params.obj;
    this.elems = {
            ...
            return {
                build: function(){
                    var count = elems.items.length,
                        i,
                        points = $( '<ul class="ares-slider1__points"></ul>' );
