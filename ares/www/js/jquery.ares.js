	...
	Ares.Portfolio.prototype = {
		...
		addSubSlider: function() {
			...
			self.elems.bigItems.each( function(){
				htmlString = '<div class="b-subGalery">\
				                <ul class="b-portfolio__using">' + $( this).find( '.b-portfolio__using' ).html() + '</ul>\
								<a href="' + $( this ).find( '>a' ).eq(0).attr( 'href' ) + '">\
									<img src="' + $( this ).find( '>a' ).eq(0).attr( 'data-pic' ) + '" width="680" height="394" >\
								</a>\
								<ul class="b-subGalery__layout">';


				$( this ).find( '> a' ).each( function(){
					htmlString += '<li class="b-subGalery__item">\
				...
				htmlString += '</ul>\
				<ul class="b-portfolio__programmers">' + $( this).find( '.b-portfolio__programmers' ).html() + '</ul>\
                </div>';
...
Ares.VerticalGalery = function( obj ) {
	...
	Ares.VerticalGalery.prototype = {
		...
		controls: function(){
			var self = this,
                items = self.obj.find( '.b-subGalery__item'),
                subMenu = self.obj.find( '.b-subGalery__layout');

			self.obj.find( '.b-subGalery__item' ).on( 'click', function() {
				if( !$( this ).hasClass( 'active' ) ) {
					self.obj.find( '.b-subGalery__item' ).removeClass( 'active' );
        ...
        startView: function(){
            ...
            if( items.length > 3 ) {
                ...
                self.obj.find('.b-subGalery__layout').append( self.scroll );
                ...
                self.scroll.slider({
                    ...
                    change: function(e,obj){
                       // self.obj.css( { display: 'none' } );
                        ...
                        self.obj.find( '.b-subGalery__item' ).css( { display: 'block' } );
                    },
		...
		schangeSlide: function( obj ) {
			...
			if( !self.move ){
				...
				self.obj.find( '.b-subGalery' ).prepend( obj.html() );
				newBlock = self.obj.find( '.b-subGalery > a' ).eq( 0 );
				startBlock = self.obj.find( '.b-subGalery > a' ).eq( 1 );
		...
		slideNext: function() {
			...
			self.obj.find('.b-subGalery__prev').css( { 'display': 'block' } );
			...
			if( !self.elems.third.next().length ) {
				self.obj.find('.b-subGalery__next').css( { 'display': 'none' } );
			}
		...
		slidePrev: function() {
			...
			self.obj.find('.b-subGalery__next').css( { 'display': 'block' } );
			...
			if( !self.elems.first.prev().length ) {
				self.obj.find('.b-subGalery__prev').css( { 'display': 'none' } );
			}
