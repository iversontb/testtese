			...
			if (el.is(':checkbox')) {
				el.css({position: 'absolute', left: -9999}).each(function() {
					if (el.next('span.jq-checkbox').length < 1) {
                        ...
                        checkbox.bind('mouseover', function(){ $(this).addClass('hovered');}).bind('mouseout', function(){ $(this).removeClass('hovered');});
						// клик на label
					...
					if (el.next('span.jq-radio').length < 1) {
						...
						radio.click(function() {
							if (!radio.is('.disabled')) {
								$('input[name="' + el.attr('name') + '"]').prop('checked', false).next().removeClass('checked').parent().removeClass('checked');
